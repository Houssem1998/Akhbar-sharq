...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-md-6">
            <form method="POST" id="event-form" action="{{ route('notification.update',[app()->getlocale(),$notification->id]) }}" enctype="multipart/form-data">
                @csrf
                @method('put')
                <h3 class="card-title">Notification Title<i class="red">*</i>:</h3>
                <div class="form-group">
                    <br /><br />
                    <input type="text" class="form-control" placeholder="Enter notification title here" id="title" name="title" value="{{ $notification->title }}">
                </div>
                <h3 class="card-title">Notification Body<i class="red">*</i>:</h3>
                <div class="form-group">
                    <br /><br />
                    <textarea type="text" class="form-control" name="body" id="" cols="30" rows="10" placeholder="Enter notification title here" id="body" name="body" >{{ $notification->body }}</textarea>
                </div>
                <div class="form-group">
                    <h3>Type:</h3>
                    <select class="form-control select2" style="width: 100%;" name="type" id="type">
                        <option @if($notification->category === 'danger') selected="selected" @endif value ="danger" >Urgent</option>
                        <option @if($notification->category === 'info') selected="selected" @endif value ="info" >Info</option>
                        <option @if($notification->category === 'warning') selected="selected" @endif value ="warning" >Warning</option>
                        <option @if($notification->category === 'seccess') selected="selected" @endif value ="success" >Seccess</option>
                    </select>
                </div>
                <!--CategorySel-->
                <div class="form-group">
                    ...
                    <select class="form-control select2" style="width: 100%;" name="category" id="category">
                        @foreach($categories as $category)
                            <option @if($category->name === $notification->category))selected="selected"@endif value ="{{ $category->name }}" >{{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                <!--/.CategorySel-->
